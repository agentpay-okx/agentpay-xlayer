...
The installer writes `AgentPayAccount.bin` and points `AGENTPAY_ACCOUNT_BYTECODE_PATH` at it. After the human fills the generated config, run `npx agentpay doctor` to check readiness without printing secrets. Run `npx agentpay setup-web` when the setup/signing page needs to be served.
