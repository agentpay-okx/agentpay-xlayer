...
create table if not exists public.setup_intents (
  ...
  home_chain_id integer not null default 196 check (home_chain_id in (196, 1952)),
...
create table if not exists public.agent_wallets (
  ...
  home_chain_id integer not null default 196 check (home_chain_id > 0),
...
create table if not exists public.payment_intents (
  ...
  source_token_symbol text not null check (source_token_symbol in ('USDT0', 'USDC', 'USDT')),
  destination_token_address text not null check (destination_token_address ~ '^0x[0-9a-fA-F]{40}$'),
  destination_token_symbol text not null check (destination_token_symbol in ('USDT0', 'USDC', 'USDT')),
